#!/usr/bin/env node
"use strict";
/*jslint unparam: true, nomen: true, node: true*/
...
var argv = require('minimist')(process.argv.slice(2), {
    string: ['listen', 'port'],
    boolean: ['help', 'hide-console', 'hide-tap'],
    alias: { h: 'help' },
});
...
function console_js(request, response) {
    var stream, b = browserify({
        debug: true,
        transformKey: ['browserify', 'tape-server-console'],
    });

    b.require('tape');  // Make tape available to test bundle
    b.add(path.join(__dirname, '/console.js'));
    ...
    // Include config so we can use it in-module
    response.write('window.tape_server = {};\n');
    response.write('window.tape_server.config = ' + JSON.stringify(argv) + ';\n');
...
function bundle_js(request, response) {
    var stream, b = browserify({
        debug: true,
        transformKey: ['browserify', 'tape-server-transform'],
    });

    b.external('tape');  // Pull tape from our bundle, so we get the writer set
    b.add(argv._);
    response.writeHead(200, {"Content-Type": "application/javascript"});
    stream = b.bundle();
    stream.once("error", function (err) {
